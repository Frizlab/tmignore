...
class TimeMachine {
	...
	static func addExclusions(paths: [String]) {
		if paths.isEmpty {
			Tmignore.logger.info("No exclusions to add")
			return
		}

		Tmignore.logger.info("Adding backup exclusions for \(paths.count) paths…")
		...
		for pathChunk in paths.chunked(by: chunkSize) {
			do {
				...
				Tmignore.logger.error("Failed to add backup exclusions: \(error.execResult.stderr ?? "")")
			}
		}

		Tmignore.logger.info("Added backup exclusions for \(paths.count) paths")
	}
	...
	static func removeExclusions(paths: [String]) {
		if paths.isEmpty {
			Tmignore.logger.info("No exclusions to remove")
			return
		}

		Tmignore.logger.info("Removing backup exclusions for \(paths.count) paths…")
		...
		for pathChunk in paths.chunked(by: chunkSize) {
			do {
				...
				if error.execResult.exitCode != 213 {
					Tmignore.logger.error("Failed to remove backup exclusions: \(error.execResult.stderr ?? "")")
				}
			}
		}

		Tmignore.logger.info("Removed backup exclusions for \(paths.count) paths")
	}
}
